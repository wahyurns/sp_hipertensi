...
echo "<h2>Hasil Perhitungan Certainty Factor (CF)</h2>";
...
while ($rpenyakit = mysqli_fetch_array($sqlpenyakit)) {
    $cftotal_temp = 0;
    $cf = 0;
    $sqlgejala = mysqli_query($conn, "SELECT * FROM basis_pengetahuan LEFT JOIN gejala ON basis_pengetahuan.kode_gejala=gejala.kode_gejala where kode_penyakit=$rpenyakit[kode_penyakit]");
    $cflama = 0;
    while ($rgejala = mysqli_fetch_array($sqlgejala)) {
        ...
        for ($i = 0; $i < count($_POST['kondisi']); $i++) {
            ...
            if ($rgejala['kode_gejala'] == $gejala) {
                ...
                echo "<tr>
                        <td>$no</td>
                        <!--<td>{$rpenyakit['kode_penyakit']}</td>-->
                        <td>{$rpenyakit['nama_penyakit']}</td>
                        <!--<td>{$rgejala['kode_gejala']}</td>-->
                        <td>{$rgejala['nama_gejala']}</td>
                        <td>" . number_format($cf, 2) . "</td>
                        <td>" . number_format($cflama, 2) . "</td>
                      </tr>";
                $no++;
            }
        }
...
// --------------------- END -------------------------
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' class='btn btn-dark' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
      ...
      echo "</table><div class='card'>
  <div class='card-body'>
    <div class='row'>
	  <div class='col-xl-6 col-xxl-6'>
		<h3>Hasil Diagnosa</h3>
		<div class='alert alert-secondary'>
			Jenis penyakit yang diderita adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round($vlpkt[1], 2) . " % (" . $vlpkt[1] . ")<br></h3>
		</div>
	  </div>
	  <div class='col-xl-6 col-xxl-6'>
  ...
  <div class='card-body'>
    <h4>";
      for ($ipl = 2; $ipl < count($idpkt); $ipl++) {
        echo "<h4><i class='fa fa-caret-square-o-right'></i> " . $nmpkt[$ipl] . " / " . round($vlpkt[$ipl], 2) . " % (" . $vlpkt[$ipl] . ")<br></h4>";
      }
    echo "</h4>
  </div>
</div>";
	 ...
	 <div class='alert alert-success alert-dismissible'>
      ...
      echo "
		  <input class='float btn btn-primary' type=submit data-toggle='tooltip' data-placement='top' title='Klik disini untuk melihat hasil diagnosa' name=submit value='&#xf00e;' style='font-family:Arial, FontAwesome'>
          </tbody></table></form>";
    }
    break;
}
