...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($conn,"SELECT * FROM admin ORDER BY username");
	echo "<br><form method=POST action='?module=admin' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
		  ...
		  <tr>
		  <td><input class='btn btn-success' type=button name=tambah value='Tambah Admin' onclick=\"window.location.href='?module=admin&act=tambahadmin';\"></td>
		  ...
		  <td><input class='btn btn-success margin' type=submit value='   Cari   ' name=Go>
		  </td> </tr>
          </table></form>";
	...
	if ($counter % 2 == 0) $warna = "dark";
       ...
       echo "<tr class='".$warna."'>
	         ...
	         <td>$r[nama_lengkap]</td>
			 <td align=center>
			 <a type='button' class='btn btn-success margin' href=?module=admin&act=editadmin&id=$r[username]><i class='fa fa-pencil-square-o' aria-hidden='true'></i> Ubah </a> &nbsp;
	          <a type='button' class='btn btn-danger margin' href=\"JavaScript: confirmIt('Anda yakin akan menghapusnya ?','$aksi?module=admin&act=hapus&id=$r[username]','','','','u','n','Self','Self')\" onMouseOver=\"self.status=''; return true\" onMouseOut=\"self.status=''; return true\"><i class='fa fa-trash-o' aria-hidden='true'></i> Hapus</a>
             </td></tr>";
      $no++;
	  $counter++;
	...
	echo "<div class=paging>";
	 ...
	 if ($offset!=0) {
			...
			echo "<span class='btn btn-square btn-outline-light btn-xs'> <a href=index.php?module=admin&offset=$prevoffset>Back</a></span>";
		}
		else {
          ...
          for ($i = 1; $i <= $halaman; $i++) {
            $newoffset = $limit * ($i - 1);
            if ($offset != $newoffset) {
              echo "<span class='btn btn-square btn-outline-light btn-xs'><a href=index.php?module=admin&offset=$newoffset>$i</a></span>";
              //cetak halaman
            } else {
          ...
          if (!(($offset / $limit) + 1 == $halaman) && $halaman != 1) {
            ...
            echo "<span class='btn btn-square btn-outline-light btn-xs'><a href=index.php?module=admin&offset=$newoffset>Next</a>";
          } else {
