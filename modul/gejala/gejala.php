...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($conn,"SELECT * FROM gejala ORDER BY kode_gejala");
	echo "<form method=POST action='?module=gejala' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
		  ...
		  <tr><td><input class='btn btn-success' type=button name=tambah value='Tambah Gejala' onclick=\"window.location.href='?module=gejala&act=tambahgejala';\"></td>
	...
	if ($counter % 2 == 0) $warna = "dark";
       ...
       echo "<tr class='".$warna."'>
			 ...
			 <td>$r[nama_gejala]</td>
			 <td align=center><a type='button' class='btn btn-success margin' href=?module=gejala&act=editgejala&id=$r[kode_gejala]><i class='fa fa-pencil-square-o' aria-hidden='true'></i> Ubah </a> &nbsp;
	...
	if ($counter % 2 == 0) $warna = "dark";
       ...
       echo "<tr class='".$warna."'>
			 ...
			 <td>$r[nama_gejala]</td>
			 <td align=center>
			 <a type='button' class='btn btn-success margin' href=?module=gejala&act=editgejala&id=$r[kode_gejala]><i class='fa fa-pencil-square-o' aria-hidden='true'></i> Ubah </a> &nbsp;
	          <a type='button' class='btn btn-danger margin' href=\"JavaScript: confirmIt('Anda yakin akan menghapusnya ?','$aksi?module=gejala&act=hapus&id=$r[kode_gejala]','','','','u','n','Self','Self')\" onMouseOver=\"self.status=''; return true\" onMouseOut=\"self.status=''; return true\"><i class='fa fa-trash-o' aria-hidden='true'></i> Hapus</a>
             </td></tr>";
      $no++;
	  $counter++;
	...
	if ($offset!=0) {
		...
		echo "<span class='btn btn-square btn-outline-light btn-xs'> <a href=index.php?module=gejala&offset=$prevoffset>Back</a></span>";
	}
	else {
	...
	for($i=1;$i<=$halaman;$i++){
		$newoffset = $limit * ($i-1);
		if($offset!=$newoffset){
			echo "<span class='btn btn-square btn-outline-light btn-xs'><a href=index.php?module=gejala&offset=$newoffset>$i</a></span>";
			//cetak halaman
		}
		else {
