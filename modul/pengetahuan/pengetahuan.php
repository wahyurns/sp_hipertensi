...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($conn,"SELECT * FROM basis_pengetahuan ORDER BY kode_pengetahuan");
	echo "<form method=POST action='?module=pengetahuan' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
		  ...
		  <tr><td><input class='btn btn-success' type=button name=tambah value='Tambah Basis Pengetahuan' onclick=\"window.location.href='?module=pengetahuan&act=tambahpengetahuan';\"></td>
	...
	$rgejala=mysqli_fetch_array($sql);
       echo "<tr class='".$warna."'>
			 ...
			 <td align=center><a type='button' class='btn btn-success margin' href=?module=pengetahuan&act=editpengetahuan&id=$r[kode_pengetahuan]><i class='fa fa-pencil-square-o' aria-hidden='true'></i> Ubah </a> &nbsp;
	...
	$rpenyakit=mysqli_fetch_array($sql2);
       echo "<tr class='".$warna."'>
			 ...
			 <td align=center>$r[md]</td>
			 <td align=center>
			 <a type='button' class='btn btn-success margin' href=?module=pengetahuan&act=editpengetahuan&id=$r[kode_pengetahuan]><i class='fa fa-pencil-square-o' aria-hidden='true'></i> Ubah </a> &nbsp;
	...
	if ($offset!=0) {
		...
		echo "<span class='btn btn-square btn-outline-light btn-xs'> <a href=index.php?module=pengetahuan&offset=$prevoffset>Back</a></span>";
	}
	else {
	...
	for($i=1;$i<=$halaman;$i++){
		$newoffset = $limit * ($i-1);
		if($offset!=$newoffset){
			echo "<span class='btn btn-square btn-outline-light btn-xs'><a href=index.php?module=pengetahuan&offset=$newoffset>$i</a></span>";
			//cetak halaman
		}
		else {
	...
	if(!(($offset/$limit)+1==$halaman) && $halaman !=1){
		...
		echo "<span class='btn btn-square btn-outline-light btn-xs'><a href=index.php?module=gejala&offset=$newoffset>Next</a>";
	}
	else {
  ...
  case "tambahpengetahuan":
	echo "	<div class='alert alert-success alert-dismissible'>
				...
				<b>Contoh:</b><br>
				Jika kepercayaan <b>(MB)</b> anda terhadap gejala pusing untuk penyakit Hipertensi Primer adalah <b>0.8 (Hampir Pasti)</b><br>
				Dan ketidakpercayaan <b>(MD)</b> anda terhadap gejala Pusing untuk penyakit  Hipertensi Primer adalah <b>0.2 (Hampir Mungkin)</b><br><br>
				<b>Maka:</b> CF(Pakar) = MB – MD (0.8 - 0.2) = <b>0.6</b> <br>
				Dimana nilai kepastian anda terhadap gejala Pusing untuk penyakit Hipertensi Primer adalah <b>0.6 (Kemungkinan Besar)</b>
              </div>
